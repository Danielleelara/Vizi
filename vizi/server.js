...
server.use((req, res, next) => {
    if (req.url === '/products' && req.method !== 'GET') {
        throw new Error('Utilizar somente GET no /products')
    }
    ...
    if (req.url === '/transactions') {
        if (req.method === 'POST') {
            ...
            throw new Error('Utilizar somente GET ou POST no /transactions')
        } else {
            setTimeout(next, 500);
        }
        ...
        if (req.method === 'POST') {
            if (
                ...
                throw new Error('Deve ser informado os campos transactionId e amount no body para o pagamento')
            }
